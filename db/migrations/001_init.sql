...
-- Minimal runnable schema for V1 (Postgres 16 recommended)
...
-- audit_log: only admin can read, everyone can insert their own audit via app layer (or admin)
...
-- outbox: allow insert by anyone (事务内)，select/update 仅 admin（实际生产建议单独 worker role）
